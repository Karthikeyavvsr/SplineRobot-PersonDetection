...
export function InteractiveSpline({
  ...
  // Smooth interpolation for robot head rotation only
  ...
  // Smooth animation loop for robot head movements only
  useEffect(() => {
    if (!splineRef.current) return

    // Only target the robot head, NOT the camera or entire robot body
    const robotHead = splineRef.current.findObjectByName('Head') ||
                      splineRef.current.findObjectByName('Top part') ||
                      splineRef.current.findObjectByName('Bot Head') ||
                      splineRef.current.findObjectByName('head') ||
                      splineRef.current.findObjectByName('Robot Head')
    ...
    if (personDetected) {
      ...
      if (!idleAnimationRef.current) {
        const startIdleAnimation = () => {
          ...
          // Gentle idle "looking around" motion (reduced for head-only)
          targetRotationRef.current = {
    ...
    const smoothUpdate = () => {
      try {
        ...
        // Apply rotation ONLY to robot head (NOT camera or body)
        if (robotHead) {
          if (isPersonNear) {
            // Mimic mode - more responsive head movement only
